/* vim: ts=4 st=4 : */
// Game Of Life with wrap around sides and top and bottom
// Fixed line 320 and 369. 
...
void main(int argc, char **argv) {
	...
	uint8_t globber = 2;    // Why did I call it this?  What does it do?
							// globber is used to set the random seed.
							// It starts off with 2 and gets increased for
							// every UP button press.
    ...
    fprintf(f, "     Wrap top and bottom &\n");
    fprintf(f, "          side to side.\n");
	...
	srand(rand()%globber);
	// The screen is 30x20
	// The play area doesn't use the bottom line. It is for text.
	int playArea[30][19];
	...
	if (playArea[29][18] == 1) {
		if (holder == 2) {
			...
			newPlayArea[29][18] = 0; } }
	// now check holder status
	if (playArea[29][18] == 0) {    // ****** might should be [29][18] 4/27/20
									// it was [29][19]
		if (holder == 3) {
			newPlayArea[29][18] = 1; 
	...
	for ( xx = 1; xx < 29; xx++) {
		...
		if (playArea[xx+1][18] == 1)
			{ holder++; }
		if (playArea[xx-1][17] == 1)   // this should be [xx-1][17], was [xx-1][18]
			{ holder++; }
		if (playArea[xx][17] == 1)
			{ holder++; }
